...
class SpellState(models.Model):
    ...
    class MovementRestriction(models.IntegerChoices):
        NONE = 0, _('None')
        MOVE_ONLY = 1, _('Cannot Move')
        PUSH_ONLY = 2, _('Cannot Push')
        SWITCH_ONLY = 3, _('Cannot Switch')
        PUSH_AND_MOVE = 4, _('Cannot Push nor Move')
        MOVE_AND_SWITCH = 5, _('Cannot Move nor Switch')
        PUSH_AND_SWITCH = 6, _('Cannot Push nor Switch')
        ALL = 7, _('No Movements')
